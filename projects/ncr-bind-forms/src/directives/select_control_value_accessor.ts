...
/**
 * @description
 * The `ControlBindValueAccessor` for writing select control values and listening to select control
 ...
 *
 * To use a select in a template-driven form, simply add an `ngBindModel` and a `name`
 ...
 *
 * ```ts
 * const selectedCountriesControl = new BindFormControl();
 * ```
 *
 * ```
 * <select [compareWith]="compareFn"  [formBindControl]="selectedCountriesControl">
...
export class NgSelectOption implements OnDestroy {
    ...
    constructor(private _element: ElementRef, private _renderer: Renderer2, @Optional() @Host() private _select: SelectControlBindValueAccessor) {
