...
/**
 * @description
 * The `ControlBindValueAccessor` for writing multi-select control values and listening to multi-select control
 * changes. The value accessor is used by the `BindFormControlDirective`, `BindFormControlName`, and `NgBindModel`
 * directives.
 *
 ...
 *
 * ```ts
 * const countryControl = new BindFormControl();
 * ```
 *
 * ```
 * <select multiple name="countries" [formBindControl]="countryControl">
 ...
 * See the `SelectControlBindValueAccessor` for usage.
 *
 ...
 *
 * @see `SelectMultipleControlBindValueAccessor`
 *
 * @ngModule BindReactiveFormsModule
 * @ngModule BindFormsModule
 * @publicApi
...
export class ɵNgSelectMultipleOption implements OnDestroy {
    ...
    constructor(private _element: ElementRef, private _renderer: Renderer2, @Optional() @Host() private _select: SelectMultipleControlBindValueAccessor) {
        if (this._select) {
